...
func TestLatencyBreakdownMatchesDedicatedReaders(t *testing.T) {
	db, err := sqliterepo.Open(filepath.Join(t.TempDir(), "usage.sqlite"))
	if err != nil {
		t.Fatalf("open database: %v", err)
	}
	...
	wantSummary, err := repo.LatencySummaryWithFilter(context.Background(), filter)
	if err != nil {
		t.Fatalf("latency summary: %v", err)
	}
	wantPoints, err := repo.LatencyPercentilesWithFilter(context.Background(), filter, "hour", time.UTC)
	if err != nil {
		t.Fatalf("latency percentiles: %v", err)
	}

	summary, points, err := repo.LatencyBreakdownWithFilter(context.Background(), filter, "hour", time.UTC)
	if err != nil {
		t.Fatalf("latency breakdown: %v", err)
	}
